...
*set the gp13 right pull up*/
	...
	{
        /* updated for regulator interface */
		struct regulator *vreg_gp13=NULL;
		int rc;
	
		vreg_gp13 = regulator_get(NULL, VREG_GP13_NAME);
		rc = regulator_set_voltage(vreg_gp13, VREG_GP13_VOLTAGE_VALUE,VREG_GP13_VOLTAGE_VALUE);
		if (rc) {
		    printk("%s: vreg_gp13  vreg_set_level failed \n", __func__);
	    }
		rc = regulator_enable(vreg_gp13);
		if (rc) {
